...
import "./UniswapV2FactoryLike.sol";
import "./UniswapV2FlashBorrowerLike.sol";
...
contract UniswapERC3156 is IERC3156FlashLender, UniswapV2FlashBorrowerLike {
    ...
    constructor(UniswapV2FactoryLike factory_, address weth_, address dai_) public {
        factory = factory_;
        weth = weth_;
        dai = dai_;
    }
     ...
     */
    function flashLoan(address receiver, address token, uint256 amount, bytes calldata userData) external  returns(bool) {
        address pairAddress = getPairAddress(token);
