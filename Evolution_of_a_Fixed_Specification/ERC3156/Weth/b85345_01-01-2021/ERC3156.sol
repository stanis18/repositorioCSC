...
contract WETH10 is IWETH10, IERC20, IERC2612, IERC3156FlashLender {
    ...
    function deposit() external  payable {
        require(address(this).balance + flashMinted <= 2**112 - 1, "WETH::deposit: supply limit exceeded");
    ...
    function depositTo(address to) external  payable {
        require(address(this).balance + flashMinted <= 2**112 - 1, "WETH::depositTo: supply limit exceeded");
    ...
    function depositToAndCall(address to, bytes calldata data) external  payable returns (bool success) {
        require(address(this).balance + flashMinted <= 2**112 - 1, "WETH::depositToAndCall: supply limit exceeded");
        balanceOf[to] += msg.value;
        emit Transfer(address(0), to, msg.value);
    ...
    function flashSupply(address token) external view  returns (uint256) {
        return token == address(this) ? 2**112 - 1 - address(this).balance - flashMinted : 0; // Can't underflow - L108
    }
    ...
    function flashLoan(address receiver, address token, uint256 value, bytes calldata data) external  {
        ...
        flashMinted += value;
        require(address(this).balance + flashMinted <= 2**112 - 1, "WETH::flashLoan: supply limit exceeded");
