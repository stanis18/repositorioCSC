...
contract WETH10 is IERC20, IERC2612, IERC3156FlashLender, IWETH10 {
    ...
    function maxFlashAmount(address token) external view  returns (uint256) {
        return token == address(this) ? 2**112 - 1 - address(this).balance - flashMinted : 0; // Can't underflow
    }
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 value, bytes calldata data) external  returns(bool) {
        ...
        require(value <= 2**112 - 1 && address(this).balance + _flashMinted + value <= 2**112 - 1, "WETH: supply limit exceeded");
    ...
    function _mintTo(address to, uint256 value) internal {
        require(address(this).balance + flashMinted <= 2**112 - 1, "WETH: supply limit exceeded");
        balanceOf[to] += value;
